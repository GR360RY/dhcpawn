...
def test_delete_subnet(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'10.100.100.0','netmask':20}))
...
def test_create_pool(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'10.0.0.0','netmask':22}))
...
def test_update_pool(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'10.0.0.0','netmask':22}))
    webapp.post('/api/subnets/', data=json.dumps({'name':'10.0.1.0','netmask':23}))
    webapp.post('/api/ranges/', data=json.dumps({'type':'dynamic','min':'10.0.0.200','max':'10.0.0.253'}))
    webapp.post('/api/pools/', data=json.dumps({'name':'test_pool_00','subnet':1,'range':1}))
...
def test_delete_pool(webapp):
    ...
    webapp.post('/api/ranges/', data=json.dumps({'type':'dynamic','min':'10.0.0.200','max':'10.0.0.253'}))
    webapp.post('/api/pools/', data=json.dumps({'name':'test_pool_01','subnet':1,'range':2}))
    webapp.delete('/api/subnets/1')
    pools = webapp.get('/api/pools/')
    assert len(pools['items']) == 0
