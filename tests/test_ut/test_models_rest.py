...
def test_update_host(webapp):
    ...
    webapp.put('/api/hosts/1', data=json.dumps({'name':'test_host_01','group':None}))
    host = webapp.get('/api/hosts/1')
...
def test_create_subnet(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':22}))
...
def test_delete_subnet(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':20}))
...
def test_create_ip(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':22}))
    webapp.post('/api/ips/', data=json.dumps({'address':'10.0.0.10','subnet':1}))
...
def test_update_ip(webapp):
    webapp.post('/api/hosts/', data=json.dumps({'name':'test_host_00','mac':'08:00:27:26:7a:e7'}))
    webapp.post('/api/ips/', data=json.dumps({'address':'10.0.0.10'}))
    webapp.put('/api/ips/1', data=json.dumps({'host':1}))
    ip = webapp.get('/api/ips/1')
    assert ip['address'] == '10.0.0.10'
    assert ip['host'] == 1
...
def test_create_range(webapp):
    webapp.post('/api/ranges/', data=json.dumps({'min':'10.0.0.200','max':'10.0.0.253','type':'static'}))
...
def test_update_range(webapp):
    ...
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':22}))
    webapp.put('/api/ranges/1', data=json.dumps({'type':'dynamic','subnet':1}))
    ip_range = webapp.get('/api/ranges/1')
...
def test_create_pool(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':22}))
    webapp.post('/api/ranges/', data=json.dumps({'type':'dynamic','min':'10.0.0.200','max':'10.0.0.253'}))
    webapp.post('/api/pools/', data=json.dumps({'name':'test_pool_00','subnet':1,'range':1}))
...
def test_update_pool(webapp):
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_00','netmask':22}))
    webapp.post('/api/subnets/', data=json.dumps({'name':'test_subnet_01','netmask':23}))
    webapp.post('/api/ranges/', data=json.dumps({'type':'dynamic','min':'10.0.0.200','max':'10.0.0.253'}))
    webapp.post('/api/pools/', data=json.dumps({'name':'test_pool_00','subnet':1,'range':1}))
    webapp.put('/api/pools/1', data=json.dumps({'name':'test_pool_01','subnet':2}))
    pool = webapp.get('/api/pools/1')
...
def test_delete_pool(webapp):
    ...
    webapp.post('/api/pools/', data=json.dumps({'name':'test_pool_01','subnet':1,'range':1}))
    webapp.delete('/api/subnets/1')
    pools = webapp.get('/api/pools/')
    assert len(pools['items']) == 0
